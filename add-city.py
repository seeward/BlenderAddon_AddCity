# needed for Blender to create the addon
bl_info = {
    "name": "CityMaker",
    "author": "seeward",
    "version": (1, 0),
    "blender": (3, 1, 0),
    "location": "View3D > Add > Add City",
...
#import other code from libraries
import bpy
import string
...
space = 3
block_space = .5
brand_names = ["/Users/seeward/Documents/Development/AddCity_BlenderAddon/AddCity/vodacom.jpeg","/Users/seeward/Documents/Development/AddCity_BlenderAddon/AddCity/santam.png", "/Users/seeward/Documents/Development/AddCity_BlenderAddon/AddCity/fnb.png"]
...
class OBJECT_OT_add_city(Operator, AddObjectHelper):
    ...
    def execute(self, context):
        ...
        # mat = bpy.data.materials[ground_tx]
        # self.add_mat(mat,bpy.context.object)
        ...
        for boxX in range(self.city_blocks):
            for boxY in range(self.city_blocks):
                ...
                bpy.ops.mesh.primitive_cube_add(
                    ...
                    scale=(1, 1, .05 ))
                # mat = bpy.data.materials[sidewalk_tx]
                # self.add_mat(mat,bpy.context.object)
                self.create_and_add_mat(bpy.context.object,0xc3c3c3,'concrete_floor')
                    ...
                    for blockX in range(1):
                        for blockY in range(1):
                            ...
                            if(self.use_trees and h == .05):
                                ...
                                self.create_and_add_mat(obj,0xc1c1c1, 'parkconcrete')
    ...
    def make_building(self, location, size, height, color, width = 2):
        ...
        bpy.ops.mesh.primitive_cube_add(
            ...
            location=location, 
            scale=(1, 1, height ))
        # tex = building_textures[ran(0,3)]
        # mat = bpy.data.materials[tex]
        # self.add_mat( mat, bpy.context.object)
        # self.create_and_add_mat(bpy.context.object,get_random_color(True),"building")
        obj = bpy.context.object
        ran_ = ''.join(random.choices(string.ascii_uppercase + string.digits, k = 10)) 

        if(ran(1,4) == 1):
            key = 'glossy'
            
            mat = self.new_shader(ran_,key,get_random_color(True))
        else:
            key = 'diffuse'
            mat = self.new_shader(ran_,key,get_random_color(True))
        
        self.add_mat(mat,obj)
        if(height != .05 and width ==1 and height >=4):
            ...
            if(width == 1 and height != .05):
                bpy.ops.mesh.primitive_plane_add(
                    ...
                    enter_editmode=False, 
                    align='WORLD', 
                    location=(location[0] + .51, (location[1] + 1.1) ,(location[2] *2)  - .5 ), 
                    scale=(1, 1,1))

                obj = bpy.context.object
                obj.scale.x = .75
                obj.scale.y = .5
                obj.rotation_euler = (radians(90), radians(0), radians(180))
                ...
                brand = brand_names[ran(0,2)]

                texImage.image = bpy.data.images.load(brand)
                mat.node_tree.links.new(bsdf.inputs['Base Color'], texImage.outputs['Color'])
                self.add_mat(mat, obj)

                # create a light data block
                light_data = bpy.data.lights.new(name="light_3", type='AREA')
                # create new object with our light datablock
                light_object = bpy.data.objects.new(name="light_2.80", object_data=light_data)
                light_object.location = (location[0] + .51, (location[1] + 1.05) ,(location[2] *2)  - .2 )
                light_object.scale.x = 2
                light_object.scale.y = 1.5
            ...
            for i in range(4):
                if(height > .05 and width != 1):
                    ...
                    bpy.ops.mesh.primitive_plane_add(
                        ...
                        enter_editmode=False, 
                        align='WORLD', 
                        location=((location[0] + .51) - 1.02, (location[1] + i / 4) - .4 ,location[2] + .1), 
                        scale=(1, 1 ,1))
                    obj = bpy.context.object
                    obj.scale.x = height - .45
                    obj.scale.y = win_size
        ...
        if(height > .05 and width != 1):
            ...
            bpy.ops.mesh.primitive_plane_add(
                enter_editmode=False, 
                align='WORLD', 
                location=(location[0], location[1] ,location[2] - ledge_offset+ .1 ), 
                scale=(1,1, 1))
    ...
    def new_shader(self, id, type, color):

        mat = self.new_material(id)
        color = rgb(color)
